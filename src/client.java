...
public class client extends JFrame implements Runnable,ActionListener {
    ...
    private JMenuItem revise = new JMenuItem("修改密码");
    private JMenuItem logout =new JMenuItem("注销用户");
    ...
    public client() throws Exception{
        ...
        Socket s = new Socket("192.168.0.0", 9999);
        ...
        while (true){
            try{
                ...
                if(strs[0].equals("LOGIN")){
                    ...
                    FileOutputStream fos = new FileOutputStream("C:\\Users\\21285\\Desktop\\应用实践基础\\"+nickName+"\\群聊记录.txt",true);
                    ...
                    jta.append(strs[1] + "注销了！山水有相逢，他日再见！\n");
                    dl.removeElement(strs[1]);
                    userList.repaint();
    ...
    public void actionPerformed(ActionEvent e) {//鼠标点击事件
        ...
        }else if(label.equals("修改密码")){
            String password = JOptionPane.showInputDialog("请输入以前的密码");
            try {
                if(match(nickName,password,1))
                {
                    String password1 = JOptionPane.showInputDialog("请输入新的密码");
                    FileReader fr = new FileReader("C:\\Users\\21285\\Desktop\\应用实践基础\\users.txt");
                    BufferedReader bf = new BufferedReader(fr);
                    replaceTxtByStr("C:\\Users\\21285\\Desktop\\应用实践基础\\users.txt",nickName+" "+password,nickName+" "+password1);
                    JOptionPane.showMessageDialog(null,"密码修改成功","成功",JOptionPane.INFORMATION_MESSAGE);
                }
                else
                {
                    JOptionPane.showMessageDialog(null, "密码错误！请重试！", "验证失败",JOptionPane.ERROR_MESSAGE);
                }
            } catch (Exception ex) {
                ex.printStackTrace();
            }

        } else if(label.equals("注销用户")) {
            String password = JOptionPane.showInputDialog("请输入你的密码" );
            try {
                if(match(nickName,password,1))
                {
                    String sure = JOptionPane.showInputDialog("确定注销账户（yes/no）？");
                    if(sure.equals("yes"))
                    {
                        FileReader fr = new FileReader("C:\\Users\\21285\\Desktop\\应用实践基础\\users.txt");
                        BufferedReader bf = new BufferedReader(fr);
                        replaceTxtByStr("C:\\Users\\21285\\Desktop\\应用实践基础\\users.txt",nickName+" "+password,"");
                        File f = new File("C:\\Users\\21285\\Desktop\\应用实践基础\\"+nickName);
                        File[] files = f.listFiles();
                        for(File file : files){
                            file.delete();
                        }
                        f.delete();
                        JOptionPane.showMessageDialog(null,"谢谢您的使用","注销成功",JOptionPane.INFORMATION_MESSAGE);
                        ps.println("LOGOUT#" + nickName);
                        System.exit(0);
                    }
                    else
                    {
                        JOptionPane.showMessageDialog(null,"继续快乐的玩耍吧！","继续使用",JOptionPane.INFORMATION_MESSAGE);
                    }
                }
                else
                {
